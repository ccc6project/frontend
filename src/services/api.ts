...
import type { Card, AuthorizationRequest, AuthorizationResponse } from '../types/card';
...
export const cardsAPI = {
  ...
  // Autorizar pago - ENDPOINT QUE SÍ TENEMOS
  async authorizePayment(authData: AuthorizationRequest): Promise<AuthorizationResponse> {
    ...
    const response = await fetch(`${API_BASE_URL}/authorization?${params}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Error en autorización');
    }
...
export const bankAPI = {
  // Lista de bancos disponibles (hardcodeada según lo que me pasaste)
  getBanks(): Bank[] {
    return [
      {
        "bank_id": 1,
